...
public class BuildWorkspaceConfigurationRetrieverTest {
    ...
    public void assertGetLatestConfgiurationOnNode() {
        ...
        WorkspaceConfiguration configuration = new WorkspaceConfiguration("serverUrl", "workspaceName", ProjectData.getProjects("projectPath", "workfolder", projects), EMPTY_CLOAKED_PATHS_LIST);
    ...
    public void assertSaveWorkspaceConfigurationUsesSaveOnBuild() throws IOException {
        ...
        when(node.getNodeName()).thenReturn("needleNode");
        ProjectData[] projects = new ProjectData[0];
        when(build.getAction(WorkspaceConfiguration.class)).thenReturn(new WorkspaceConfiguration("serverUrl", "workspaceName", ProjectData.getProjects("projectPath", "workfolder", projects), EMPTY_CLOAKED_PATHS_LIST));
